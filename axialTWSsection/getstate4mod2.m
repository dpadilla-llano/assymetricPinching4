...
%         state4Strain(3) = hghTstateStrain*MDL.rDispP;
        state4Strain(3) = TperElong*(1 + MDL.rDispP);
        if (MDL.uForceP == 0.0)
        ...
        if ((state4Stress(4)-state4Stress(3))/(state4Strain(4)-state4Strain(3)) > kElasticPosDamgd) 
            ...
            % disp('Corrected Reload Stiffness to kElasticPosDamgd')
        end
    ...
    % final check	
    i = 1;		
    while (i<4) % Enforces monotonic Increasing Load-Response through State 4
        % du and df are local variables
...
%             i = 4;
%         end
        %slope12 = (state4Stress(2)-state4Stress(1))/(state4Strain(2)-state4Strain(1));% = kunload
        slope12 = kunload;
        b12 = state4Stress(1) - slope12*state4Strain(1);
%         if ((-b12/slope12) >= 0)
            disp(['Elastic Elongation = ' num2str((-b12/slope12))])
            if (du<0.0 || df<0.0) && ((-b12/slope12) >= 0)
                du = state4Strain(4)-state4Strain(1);
                df = state4Stress(4)-state4Stress(1);
                state4Strain(2) = state4Strain(1) + 0.33*du;
                state4Strain(3) = state4Strain(1) + 0.67*du;
